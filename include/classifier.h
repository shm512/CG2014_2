...
class TModel
{
 ...
 public:
    ...
    // Save model to file
    void
    Save(const string &model_file) const
    {
        assert(model_.get());
...
class TClassifier 
{
 ...
 public:
    ...
    // Basic constructor
    TClassifier(const TClassifierParams &params)
        : params_(params)
    {}

    // Train classifier
    void
    Train(const TFeatures &features, TModel *model)
    {
        ...
        for (size_t sample_idx = 0; sample_idx < number_of_samples; ++sample_idx) {
            assert(features[sample_idx].first.size() == number_of_features);
            ...
            for (unsigned int feature_idx = 0; feature_idx < number_of_features; feature_idx++) {
                ...
                prob.x[sample_idx][feature_idx].value = 
                    features[sample_idx].first[feature_idx];
            }
    ...
    // Predict data
    void
    Predict(const TFeatures& features, const TModel& model, TLabels* labels) 
    {
        // Number of samples and features must be nonzero
        size_t number_of_samples = features.size();
        assert(number_of_samples > 0);
