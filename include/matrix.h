...
public:
	...
	Matrix<ValueT>
    extra_borders(uint kernel_vert_radius, uint kernel_hor_radius) const;
	...
	// Cell matrix convolution.
	//
	// You give this function a unary operator. Operator _must_
	// have fields row_split_c and col_split_c and function
	// operator()(const Matrix<ValueT> submatrix).
    // Function applies operator to every of row_split_c x col_split_c 
    // submatrices, which have almost equal size and do not override
    // Result is stored in *results (whatever it is) by operator itself
    template<typename CellMatrixOperator, typename ResultType>
    void
    cell_map(const CellMatrixOperator &op, ResultType *results) const;
	...
	friend Matrix<ValueT> binary_map(
        const BinaryMatrixOperator &,
		const Matrix<ValueT> &,
		const Matrix<ValueT> &);
