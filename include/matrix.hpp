...
template<typename T>
inline T&
Matrix<ValueT>::make_rw(const T& val) const
{
...
template<typename ValueT>
const Matrix<ValueT>
Matrix<ValueT>::submatrix(
    uint prow, uint pcol,
	uint rows, uint cols
    ) const
{
	if (prow + rows > n_rows || pcol + cols > n_cols) {
        //let's return submatrix of max available size
		rows = n_rows - prow;
        cols = n_cols - pcol;
        //other option: throw std::string("Out of bounds");
    }
...
Matrix<ValueT>::unary_map(const UnaryMatrixOperator &op) const
{
	...
	for (uint i = 0; i < n_rows; ++i) {
		for (uint j = 0; j < n_cols; ++j) {
			auto neighbourhood = 
                extra_image.submatrix(
                    i, j,
                    2 * kernel_vert_radius + 1,
                    2 * kernel_hor_radius + 1);
...
template<typename ValueT>
template<typename CellMatrixOperator, typename ResultType>
void
Matrix<ValueT>::cell_map(const CellMatrixOperator &op, ResultType *results) const
{
    double row_split_step = (n_rows + 0.1) / op.row_split_c;
    double col_split_step = (n_cols + 0.1) / op.col_split_c;
    
    for (double i = 0; i < n_rows; i += row_split_step) {
        for (double j = 0; j < n_cols; j += col_split_step) {
            op(
                submatrix(i, j, row_split_step, col_split_step),
                results
            );
        }
    }
...
template<typename ValueT>
Matrix<ValueT>
Matrix<ValueT>::extra_borders(
    uint kernel_vert_radius,
    uint kernel_hor_radius
    ) const
{
	Matrix<ValueT> extra_image =
        Matrix<ValueT>(
            n_rows + 2 * kernel_vert_radius,
            n_cols + 2 * kernel_hor_radius
        );
	for (uint i = 0; i < n_rows; i++) {
		for (uint j = 0; j < n_cols; j++) {
			extra_image(i + kernel_vert_radius, j + kernel_hor_radius) = 
                (*this)(i, j);
		}
	}
	//top and bottom
	for (uint i = 0; i < kernel_vert_radius; i++) {
		for (uint j = 0; j < n_cols; j++) {
			extra_image(kernel_vert_radius - i - 1, j + kernel_hor_radius) = 
                (*this)(i, j);
			extra_image(n_rows + i + kernel_vert_radius, j + kernel_hor_radius) = 
                (*this)(n_rows - 1 - i, j);
		}
	}
	//left and right
	for (uint i = 0; i < n_rows; i++) {
		for (uint j = 0; j < kernel_hor_radius; j++) {
			extra_image(i + kernel_vert_radius, kernel_hor_radius - j - 1) = 
                (*this)(i, j);
			extra_image(i + kernel_vert_radius, n_cols + kernel_hor_radius + j) = 
                (*this)(i, n_cols - 1 - j);
		}
	}
	//corners
	for (uint i = 0; i < kernel_vert_radius; i++) {
		for (uint j = 0; j < kernel_hor_radius; j++) {
			//top-left
			extra_image(kernel_vert_radius - i - 1, kernel_hor_radius - j - 1) =
                (*this)(i, j);
			//bottom-right
			extra_image(n_rows + kernel_vert_radius + i, n_cols + kernel_hor_radius + j) =
                (*this)(n_rows - 1 - i, n_cols - 1 - j);
			//top-right
			extra_image(kernel_vert_radius - i - 1, n_cols + kernel_hor_radius + j) =
                (*this)(i, n_cols - 1 - j);
			//bottom-left
			extra_image(n_rows + kernel_vert_radius + i, kernel_hor_radius - j - 1) =
                (*this)(n_rows - 1 - i, j);
		}
	}
	return extra_image;
